...
const AuthenticatedLayout = ({ children, currentPath }: AuthenticatedLayoutProps) => {
  ...
  const classNames = (href: string) => clsx('button full', currentPath === href && 'active');
    ...
    <div className="authenticated-layout">
      ...
      <main>
        <aside>
          ...
          <Link href={route('fellowships')} className={classNames('fellowships')}>
            <RiTeamLine size={18} />
          ...
          <Link href={route('photoLibrary')} className={classNames('photoLibrary')}>
            <RiImageLine size={18} />
            {message.admin.menu.photoLibrary}
          </Link>
          <hr className="my-1" />
          <Link href={route('profile.edit')} className={classNames('profile.edit')}>
            <RiUserLine size={18} />
